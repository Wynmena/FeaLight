...
func toggle_menu() -> void:
	if sys_menu_instance == null:
		...
		$SystemEnum.add_child(sys_menu_instance)
...
func _on_tree_ready():
	...
	Manager.BugMgr.add_item_from_id(1, 1)
	#for item in Manager.BugMgr.get_all_item():
		#print(item.name, "  ", item.icon_path)
	Manager.BugMgr.add_item_from_id(2, 1)
	#for item in Manager.BugMgr.get_all_item():
		#print(item.name, "  ", item.icon_path)
	Manager.BugMgr.add_item_from_id(3, 1)
	#for item in Manager.BugMgr.get_all_item():
		#print(item.name, "  ", item.icon_path)
